...
var _ = Describe("Retirement", func() {
	Context("given input from user should calculate if / when retirement goals are met", func() {
		...
		It("should return an int of age when goals are met ", func() {
			...
			Expect(age).To(BeAssignableToTypeOf(int64(1)))
		})
		...
		It("should return the correct age when goals reached", func() {
			...
			Expect(age).To(Equal(int64(40)))
			_, age = CalculateRetirement(30, 10000, 300000)
			Expect(age).To(Equal(int64(60)))
		})
